...
public class SwerveSubsystem extends SubsystemBase {
   ...
   */
  public Command aimAtTarget(String camera) {
    return run(() -> {
      if (LimelightHelpers.getTV(camera)) {
        drive(getTargetSpeeds(0,
            0,
            Rotation2d.fromDegrees(LimelightHelpers.getTX(camera)))); // Not sure if this will work, more math may be required.
      }
    });
  }
...
// Since AutoBuilder is configured, we can use it to build pathfinding commands
  ...
  public Command driveCommandNonRel(DoubleSupplier translationX, DoubleSupplier translationY, DoubleSupplier angularRotationX) {
    return run(() -> {
      // Make the robot move
      swerveDrive.drive(new Translation2d(Math.pow(translationX.getAsDouble(), 3) * swerveDrive.getMaximumVelocity(),
              Math.pow(translationY.getAsDouble(), 3) * swerveDrive.getMaximumVelocity()),
